...
public:
	...
	//Greedy approach. Selects the cell with the minimum number of solutions
	bool findBestCell(int &best_i, int &best_j);

	//Check if the number n could be accepted in the i y cell
    bool accepts(int i, int j, int n);
	//Proceds playing
	void setter(int i, int j, int n);

    void undo(int i, int j, int n);
};
